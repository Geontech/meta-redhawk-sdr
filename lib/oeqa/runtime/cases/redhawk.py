...
class Runtime(RHRuntimeTestCase):
    ...
    @OEHasPackage(['redhawk'])
    def test_redhawk_user(self):
        out_uid = self.run_command('id -u redhawk')
        msg = 'REDHAWK UID did not match configuration'
        self.assertEqual(out_uid, self.td.get('REDHAWK_UID'), msg)

        out_gid = self.run_command('id -g redhawk')
        msg = 'REDHAWK GID did not match configuration'
        self.assertEqual(out_gid, self.td.get('REDHAWK_GID'))

        user, group = self.run_command("bash -lc 'ls -ld $SDRROOT' | awk '{print $3} {print $4}'").split()
        msg = 'REDHAWK "redhawk" user does not own SDRROOT'
        self.assertEqual(user, 'redhawk', msg)
        msg = 'REDHAWK "redhawk" group does not own SDRROOT'
        self.assertEqual(group, 'redhawk', msg)
