...
# Our dynamic do_patch tasks are sensitive to PACKAGE_ARCH and the
...
do_unpack[vardeps] += "PACKAGE_ARCH"
...
do_autotools_patch () {
    ...
    sed -i -r "s,(bindir = .+?cpp)/?,\1-${PACKAGE_ARCH}/,g" ${S}/Makefile.am

    # Patch the libdir to match what the spd patch will do to the entrypoint
    sed -i -r "s,(libdir = .+?cpp)/?,\1-${PACKAGE_ARCH}/,g" ${S}/Makefile.am
